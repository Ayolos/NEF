    ...
    <AppLayout>
        ...
        <div v-if="data" class="md:px-14 px-5 mt-20 md:mt-40 mb-20 md:mb-30 flex flex-col gap-20">
            <div>
                ...
                <BigCard>
                    <template #section1>
                        <img v-if="data?.homePageCollection?.items[0]?.porteSectionelle?.url" :src="data?.homePageCollection?.items[0]?.porteSectionelle?.url" alt="" class="w-1/2 h-max mx-auto mt-2 rounded-lg">
                    </template>
                    <template #section2>
                        <img v-if="data?.homePageCollection?.items[0]?.rideauMetallique?.url" :src="data?.homePageCollection?.items[0]?.rideauMetallique?.url" alt="" class="w-1/2 h-max mx-auto mt-2 rounded-lg">
                    </template>
                    <template #section3>
                        <img v-if="data?.homePageCollection?.items[0]?.grilleEnroulement?.url" :src="data?.homePageCollection?.items[0]?.grilleEnroulement?.url" alt="" class="w-1/2 h-max mx-auto mt-2 rounded-lg">
                    </template>
                    <template #section4>
                        <img v-if="data?.homePageCollection?.items[0]?.porteRapide?.url" :src="data?.homePageCollection?.items[0]?.porteRapide?.url" alt="" class="w-1/2 h-max mx-auto mt-2 rounded-lg">
                    </template>
                    <template #section5>
                        <img v-if="data?.homePageCollection?.items[0]?.niveleurQuai?.url" :src="data?.homePageCollection?.items[0]?.niveleurQuai?.url" alt="" class="w-1/2 h-max mx-auto mt-2 rounded-lg">
                    </template>
                    <template #section6>
                        <img v-if="data?.homePageCollection?.items[0]?.sas?.url" :src="data?.homePageCollection?.items[0]?.sas?.url" alt="" class="w-1/2 h-max mx-auto mt-2 rounded-lg">
                    </template>
                </BigCard>
            </div>
